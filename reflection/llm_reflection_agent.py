...
class LLMReflectionAgent:
    ...
    def is_configured(self) -> bool:
        ...
        if self.backend == "ollama" or self.backend == "airllm" or self.backend == "lmstudio":
            return True
        return False
    ...
    def _query_llm(
        ...
        if self.backend == "airllm":
            return self._query_airllm(scene_info, policy, rgb, history)
        if self.backend == "lmstudio":
            return self._query_lmstudio(scene_info, policy, rgb, history)
        if self.backend == "openai":
            return self._query_openai(scene_info, policy, rgb, history)
        raise RuntimeError(f"Unsupported backend: {self.backend}")

    def _query_openai(self, scene_info, policy, rgb, history) -> LLMDecision:
        prompt = self._build_prompt(scene_info, policy, history)
        content = [{"type": "text", "text": prompt}]
        image_url = self._rgb_to_openai_image_url(rgb) if self.use_vision else None
        if image_url:
        ...
        response_text = self._post_json(self.endpoint, payload, headers, label="openai")
        data = json.loads(response_text)
        decision = self._parse_decision(data["choices"][0]["message"]["content"], mode="openai")
    ...
    def _query_lmstudio(self, scene_info, policy, rgb, history) -> LLMDecision:
        prompt = self._build_prompt(scene_info, policy, history)
        user_message = {"role": "user", "content": prompt}
        image_b64 = self._rgb_to_base64_jpeg(rgb) if self.use_vision else None
        if image_b64:
        ...
        headers = {"Content-Type": "application/json"}
        response_text = self._post_json(self.endpoint, payload, headers, label="lmstudio")
        data = json.loads(response_text)
        decision = self._parse_decision(data["choices"][0]["message"]["content"], mode="lmstudio")
        self._log_llm_decision(scene_info, policy, decision)
        return decision

    def _query_airllm(self, scene_info, policy, rgb, history) -> LLMDecision:
        print("[INFO] airllm call started")
        ...
        try:
            ...
            print(f"[WARNING] airllm server at {base_url} did not respond to ping, but will continue")

        try:
            prompt = self._build_prompt(scene_info, policy, history)
            # AirLLM server only expects messages currently; it does not support images
            payload = {
                "messages": [
                    {"role": "system", "content": self._system_prompt()},
            ...
            headers = {"Content-Type": "application/json"}
            response_text = self._post_json(base_url, payload, headers, label="airllm", endpoint=endpoint)
            print("[INFO] airllm call completed")
            ...
            decision = self._parse_decision(text, mode="airllm")
            self._log_llm_decision(scene_info, policy, decision)
            return decision
        except Exception as error:
            print(f"[ERROR] airllm server not reachable at {full_url}")
            print("make sure airllm_server.py is running")
            fallback = self._fallback(scene_info)
            fallback.mode = "airllm"
            return fallback
