...
try:
    ...
    LLMReflectionAgent = None
    choose_recovery_strategy = None
    execute_recovery = None
    print("LLM recovery stack unavailable:", import_error)
...
timer = 0
stable_counter = 0
llm_agent = LLMReflectionAgent() if LLMReflectionAgent is not None else None
...
def get_current_joint_angles():
    ...
    for joint_index in range(p.getNumJoints(robot)):
        ...
        if joint_info[2] in [p.JOINT_REVOLUTE, p.JOINT_PRISMATIC]:
            joint_positions.append(float(p.getJointState(robot, joint_index)[0]))
...
def trigger_llm_recovery(failure_type, task_stage, camera_observation):
    if llm_agent is None or choose_recovery_strategy is None or execute_recovery is None:
        print("Skipping LLM recovery: planner or reflection agent is unavailable.")
        return

    robot_state = {
        "joint_angles": get_current_joint_angles(),
        "gripper_force": estimate_gripper_force(),
    ...
    policy = {
        "x_offset": float(retry_offset[0]),
        "y_offset": float(retry_offset[1]),
        "grasp_height": float(current_target[2]),
        "approach_height": float(target[2]),
        "lift_height": 0.0,
        "release_delay": 0,
    }
    ...
    try:
        ...
        strategy = choose_recovery_strategy(llm_response, failure_type, robot_state)
        print("Chosen strategy:", strategy)
        execute_recovery(strategy, robot_state)
    except Exception as runtime_error:
        print("LLM recovery failed:", runtime_error)
    ...
    if ord(' ') in keys and keys[ord(' ')] & p.KEY_WAS_TRIGGERED:
        ...
        if constraint_id is None:
            ...
            if dist < 0.07:
                ...
                print("Grasp FAILED")
                failure_image_path = capture_failure_image("grasp_fail")
                trigger_llm_recovery(
                    failure_type="grasp_failure",
                    task_stage="grasp",
                    camera_observation=failure_image_path,
                )
