...
# Supported values: "ollama", "openai", and "airllm".
...
# ---------------- CUBE ----------------
# Random starting position for true autonomy testing
import random
random_x = random.uniform(-0.2, 0.2)
random_y = random.uniform(-0.2, 0.2)
cube = p.loadURDF("cube_small.urdf", [random_x, random_y, 0.02])
print(f"Cube placed at random position: ({random_x:.3f}, {random_y:.3f}, 0.02)")
p.changeDynamics(
    cube,
    -1,
    lateralFriction=1.5,
...
# ---------------- SMOOTH MOTION ----------------
def smooth_move(target_pos, steps=300):
    joint_indices = []
    current_positions = []
...
agent = LLMReflectionAgent(
    backend=os.getenv("LLM_AGENT_BACKEND", "lmstudio"),
    model=os.getenv("LLM_AGENT_MODEL", "qwen2.5-7b-instruct-1m"),
    endpoint=os.getenv("LLM_AGENT_ENDPOINT", "http://localhost:1234/v1/chat/completions"),
